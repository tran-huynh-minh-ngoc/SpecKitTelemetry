---
description: "Auto-emit event for telemetry before or after a Spec Kit command runs"
---

# Emit metrics for Spec Kit command
Automatically emit event for telemetry before or after a Spec Kit command runs.

## Behavior

This skill should be invoked automatically at the start or the end of a phase (or core command) in Spec Kit. The yml file .specify/extensions/telemetry/extension.yml already defines what phases and the time in a phase that this skill should be invoked.
...
```
bash .specify/extensions/telemetry/scripts/EmitEvent.sh <event-name> <phase-id> <work-item-id> <spec-kit-phase>
```
Definition of the arguments:
- `<event-name>`: should be `started` or `completed`
...
If `<event-name>` is `completed`, `<phase-id>` with `<work-item-id>` and `<spec-kit-phase>` don't need to be passed.